...
public class AclAuthorizationHandlerTest {
    ...
    public void compareDenyWinsAtSamePrecision() {
        AclAuthorizationHandler comparator = new AclAuthorizationHandler(new AuthConfig());
        PolicyEntry allow = policyEntry("Topic:test", Decision.ALLOW);
        PolicyEntry deny = policyEntry("Topic:test", Decision.DENY);

        Assert.assertTrue(comparator.comparePolicyEntries(deny, allow) < 0);
        Assert.assertTrue(comparator.comparePolicyEntries(allow, deny) > 0);

        List<PolicyEntry> entries = new ArrayList<>(Arrays.asList(allow, deny));
        entries.sort(comparator::comparePolicyEntries);
    ...
    public void compareKeepsComparatorContract() {
        ...
        Assert.assertEquals(0, comparator.comparePolicyEntries(deny1, deny2));
        Assert.assertEquals(0, comparator.comparePolicyEntries(allow1, allow2));
        ...
        for (PolicyEntry e1 : entries) {
            for (PolicyEntry e2 : entries) {
                Assert.assertEquals(Integer.signum(comparator.comparePolicyEntries(e1, e2)),
                    -Integer.signum(comparator.comparePolicyEntries(e2, e1)));
            }
        }
    }
    ...
    public void comparePrecisionBeforeDecision() {
        ...
        Assert.assertTrue(comparator.comparePolicyEntries(literalAllow, prefixedDeny) < 0);
        Assert.assertTrue(comparator.comparePolicyEntries(prefixedDeny, literalAllow) > 0);
        Assert.assertTrue(comparator.comparePolicyEntries(prefixedDeny, anyDeny) < 0);

        List<PolicyEntry> entries = new ArrayList<>(Arrays.asList(anyDeny, prefixedDeny, literalAllow));
        entries.sort(comparator::comparePolicyEntries);
